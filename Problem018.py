...
def main():
    ...
    for x in range(len(triangle)):
        for y in range(len(triangle[x])):
            print("Row", x, "column", y, "holds", triangle[x][y])
    ...
    for x in reversed(range(len(triangle) - 1)):
        ...
        for n in range(len(triangle) - 1):
            # if len(step_values) == x:
            #     continue
            try:
                left_val = triangle[x][n] + triangle[x+1][n]
                print("adding", triangle[x][n], "to", triangle[x+1][n], "for left val", left_val)
                right_val = triangle[x][n] + triangle[x+1][n+1]
                print("adding", triangle[x][n], "to", triangle[x+1][n+1], "for right val", right_val)
            except IndexError:
                pass
            if left_val > right_val:
                ...
                step_values.append(left_val)
            print("ending with largest step val of", largest_step_val)
        print("replacing row with", step_values)
